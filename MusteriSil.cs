    ...
    public partial class MusteriSil : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (dr.Read())
            {
                ...
                dr.Close();
                DialogResult durum = MessageBox.Show(" MÜSTERİYİ SİLMEK İSTİYOR MUSUNUZ?", "SİL", MessageBoxButtons.YesNo);
                if (DialogResult.Yes == durum)
                {
                    string sil = "DELETE from MUSTERİ where TC=@tc";
                    SqlCommand cmdsil = new SqlCommand(sil, con);
                    cmdsil.Parameters.AddWithValue("@tc", textBox1.Text);
                    cmdsil.ExecuteNonQuery();
                    MessageBox.Show("MUSTERİ SİLME BAŞARILI!");
                }
            }
            else
